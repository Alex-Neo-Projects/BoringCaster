...
export default function CastCard({cast}) {
  const timestamp = moment.unix(cast.timestamp / 1000).fromNow();
      ...
      <View>
        {parts.map((part, index) => {
          ...
          if (matches.includes(part)) {
            return <ClickableImage key={index} source={part}/> 
      ...
      <View style={{flex: 5}}>
        <View style={{flexDirection: 'row'}}>
          <Text style={{fontSize: 14, paddingBottom: 6, flex: 1}}>
            <Text style={{fontWeight: 'bold', fontSize: 17}}>{cast.author.displayName}</Text> 
            <Text style={{color: '#373737', fontSize: 17}}> @{cast.author.username} •</Text>
            <Text style={{color: '#373737', fontSize: 16}}> {timestamp}</Text>
          </Text>
        </View>
        <View>
